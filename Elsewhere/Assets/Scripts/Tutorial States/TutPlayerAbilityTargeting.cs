...
public class TutPlayerAbilityTargeting : PlayerAbilityTargeting
{
    Ability ability;
    ...
    {
        turnScheduler.abilityChoosingDialogue.SetActive(false);
        turnScheduler.abilityExecutingDialogue.SetActive(true);
        if (!tile.attackable)
        {
            yield break;
        }
    ...
    { 
        turnScheduler.abilityClickDialogue.SetActive(false);
        turnScheduler.confirmationPanel.SetActive(false);
        turnScheduler.cancelPanel.SetActive(false);
        GameAssets.MyInstance.highlightMap.RemoveClicked();
        turnScheduler.SetState(new TutPlayerAbility(turnScheduler));
        yield break;
    }
